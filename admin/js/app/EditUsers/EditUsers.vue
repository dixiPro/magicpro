...
onMounted(() => {
    ...
    // глобальные сервисы
    // тосты
    // выводит тост уже готов,  не надо ничего монтировать и вызывать
    // использование document.confirmDialog('Сохранить?');
    ...
    // диалог подтверждения используется для задания вопроса Да или Нет
    // выводит тост уже готов,  не надо ничего монтировать
    // использование document.showToast('Сообщение');
...
<template>
    ...
    <Dialog v-model:visible="dialogUserForm.show" :header="dialogUserForm.header" modal style="width: 400px">
        ...
        <button class="btn btn-sm btn-success" v-if="dialogUserForm.mode === 'edit'"
            @click="editUser">Сохранить</button>
