...
<template>
  <Dialog v-model:visible="store.statusFileManager" modal position="top" class="w-50 mt-4">
    <template #header>
    ...
    <div v-if="ready" class="mt-2">
      ...
      <div :class="{ 'd-flex flex-wrap': viewFull }" class="">
        <template v-for="(el, index) in directory.filter((e) => e.type !== 'dir')">
          <div v-if="!viewFull" class="d-flex my-1" @contextmenu.prevent="onRightClick($event, el)">
            ...
            <div class="">
              ...
              <span v-if="el.isImage"
                ><span class="ms-1" style="font-size: 85%">({{ el.width }}x{{ el.height }})</span></span
              >
            </div>
          </div>
          <div v-else class="thumb-pic" :style="'width: ' + viewSize + 'px; height: ' + viewSize + 'px;'" @contextmenu.prevent="onRightClick($event, el)">
