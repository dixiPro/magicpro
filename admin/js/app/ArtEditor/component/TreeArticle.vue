...
const changeNode = (node) => {
    walkTree(treeData.value[0], (n) => {
        if (n?.id === node.id) {
            n.text = node.title;
            n.menuOn = node.menuOn;
            n.isRoute = node.isRoute;
        }
    });
};

// изменить узел, например, название поменялось
defineExpose({
    changeNode,
});

// текущий id в дереве
const currentNodeId = ref(props.idArticle);

watch(currentNodeId, () => {
    createEmmit("changeActiveArt", currentNodeId.value);
});
...
const createEmmit = (command, value) => {
    emit("tree", { command: command, value: value });
};
