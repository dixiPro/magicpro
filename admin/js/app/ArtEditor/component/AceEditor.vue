...
const getMode = () =>
    props.lang === "php" ? "ace/mode/php" : "ace/mode/html"; const getTheme = (theme) => `ace/theme/${theme}`;
...
watch(() => props.lang, (v) => editor?.session.setMode(getMode(v)));
watch(() => props.theme, (v) => editor?.setTheme(getTheme(v)));
watch(() => props.readOnly, (v) => editor?.setReadOnly(v));
watch(() => props.tabSize, (v) => editor?.session.setTabSize(v));
watch(() => props.wrap, (v) => editor?.session.setUseWrapMode(v));
</script>
