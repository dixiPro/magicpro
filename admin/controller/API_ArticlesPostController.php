...
use MagicProAdminControllers\PintFormatter;
...
class API_ArticlesPostController extends Controller
{
    ...
    private function format(Request $request): array
    {
        ...
        $fileName = MAGIC_VIEW_DIR . '/' . $request->string('name')->toString() . '.blade.php';
        $formatter = new PintFormatter();
        $result = $formatter->formatFile($fileName);
        return ['code' => $result];
    }
