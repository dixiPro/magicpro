# Центральный модуль: как пользоваться

Документ для того, кто пишет страницы: заводит статьи, настраивает маршруты и
верстает блейды. Как устроен движок внутри — в `inside.md`.
...
Полный список с параметрами и примерами — в админке, `/a_dmin/documentation`.
Он собирается из phpdoc самих методов, поэтому не расходится с кодом.
...
Каждое сохранение статьи оставляет её копию. Кнопка с часами в редакторе,
рядом с лампочкой, открывает список версий: когда и кто. Тык по строке —
открывается сохранённая статья, блейд и контроллер видно целиком и можно забрать
из них кусок. Внизу «Восстановить» и «Выход».

Восстановление — это обычное сохранение: файлы перегенерируются, а статья, какая
была до отката, остаётся в архиве.

Что стоит знать:

- **Сохранение без изменений версии не создаёт.** Три сейва подряд — одна строка.
- **Сохранения агента через МСП попадают в архив так же,** и в колонке «кто»
  стоит владелец токена.
- **Импорт архив не наполняет** — у него свои снимки всего дерева.
- **Сколько хранить — настройка `ARCHIVE_DAYS`** на странице настроек, по
  умолчанию 30 дней, `0` значит вечно. Самая свежая версия статьи не удаляется
  никогда.
- **Версии удалённых статей остаются.** Снести их можно кнопкой на
  `/a_dmin/setup`.

## Чего делать не надо

- Править файлы в `storage/dataMagicPro/` — они генерируются и перезаписываются.
- Менять статью прямым SQL: сохранение перегенерирует файлы и пересчитывает
  `npp` и `directory` в транзакциях, а `UPDATE` мимо API рассинхронит базу с
  диском.
- Заводить в имени статьи что-то кроме латинских букв, цифр, дефиса и
  подчёркивания — маршрут до неё не дойдёт.
